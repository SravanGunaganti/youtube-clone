// Import controller functions to handle channel-related routes
import { 
  createChannel, 
  updateChannel, 
  deleteChannel, 
...
export function channelRoutes(app) {
  ...
  // Subscription routes
  app.post("/api/channels/:channelId/subscribe", 
    authenticateToken, 
    validateObjectId('channelId'), 
    toggleSubscribe
  );
  
  app.get("/api/channels/:channelId/subscription-status", 
    authenticateToken, 
    validateObjectId('channelId'), 
    checkSubscriptionStatus
  );
